...
?>
<a id="tfk_back_list" href="<?php echo remove_query_arg(array('view', 'noheader')).'#tfk_row-'.$iid ?>"><?php _e('Back to list', 'taskfreak') ?></a>
...
<div id="tfk_task_edit_comment">
	...
	<a id="tfk_task_edit" href="<?php echo remove_query_arg(array('view','noheader'), add_query_arg('edit', $iid)) ?>#tfk_edit_task_form">
...
<div id="tfk_task_details" class="tfk_pr<?php echo $this->data->get('priority') ?>">
	...
	<p class="tfk_sts">
		...
		<a id="tfk_sts1-<?php echo $iid ?>" 
			...
			href="<?php echo remove_query_arg('view', add_query_arg(array('edit' => $iid, 'status' => 20, 'tfknonce' => $nonce))); ?>" 
			...
			href="<?php echo remove_query_arg('view', add_query_arg(array('edit' => $iid, 'status' => 30, 'tfknonce' => $nonce))); ?>" 
			...
			href="<?php echo remove_query_arg('view', add_query_arg(array('edit' => $iid, 'status' => 60, 'tfknonce' => $nonce))); ?>" 
...
<?php while ($this->comment->next()): ?>
	<li class="tfk_comment" id="tfk_comment_<?php echo $this->comment->get_uid() ?>">
		...
		<div>
			<a class="tfk_comm_lnk" href="<?php echo add_query_arg('view', $iid, tzn_tools::baselink()) ?>#tfk_comment_<?php echo $this->comment->get_uid() ?>">#</a>
...
<?php if ($this->user_can_comment_project): ?>
<form id="tfk_comment_form" action="<?php echo add_query_arg('noheader', 1) ?>#tfk_task_title" enctype="multipart/form-data" method="post">
